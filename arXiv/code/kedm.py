...
def KEDM(param, A):
    ...
    for k in range(K):
        ...
        if k==0:
            constraint = cvx.normNuc(G[k])
        else:
            constraint = constraint + cvx.normNuc(G[k])
    ...
    Lambda = .001
    obj = cvx.Minimize(cost+Lambda*constraint)
    prob = cvx.Problem(obj,con)

    try:
        prob.solve(solver=cvx.CVXOPT)#, normalize = False)#solver=cvx.SCS, verbose = True)#,verbose=False,normalize = True)
